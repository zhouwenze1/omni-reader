...
void main() {
  group('EpubTocReconciler', () {
    test('adds orphaned spine items under the nearest section parent', () async {
    ...
    test('keeps missing top-level chapters as siblings when no section parent exists',
        () async {
      const reconciler = EpubTocReconciler();
      final source = _MemoryResourceSource(
        files: <String, String>{
    ...
    test('builds fallback toc from spine when navigation is missing', () async {
      ...
      final source = _MemoryResourceSource(
        files: <String, String>{
          ...
          'OEBPS/Text/ch2.xhtml':
              '<html><body><h1>Middle</h1></body></html>',
        },
      );
      ...
      expect(reconciled.map((item) => item.title), <String>['Opening', 'Middle']);
